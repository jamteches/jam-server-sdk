  ...
  dynamic _handleResponse(http.Response response) {
    if (response.statusCode >= 200 && response.statusCode < 300) {
      ...
      try {
        ...
        // Try to get retry_after from body or header
        if (decoded['retry_after'] != null) {
          retryAfter = decoded['retry_after'] is int ? decoded['retry_after'] : int.tryParse(decoded['retry_after'].toString());
        }
      } catch (e) {
        message = response.body;
      }
      ...
      if (retryAfter == null && response.headers.containsKey('retry-after')) {
...
class JamException implements Exception {
  ...
  @override
  String toString() => 'JamException(statusCode: $statusCode, message: $message, retryAfter: $retryAfter)';
